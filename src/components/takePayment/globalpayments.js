export var GlobalPayments = (function () {
	...
	***************************************************************************** */
	...
	var PostMessage = /** @class */ (function () {
	    ...
	    PostMessage.prototype.post = function (data, target) {
	        ...
	        var win = target === "parent" ? window.parent : targetNode.contentWindow || targetNode;
	...
	function objectAssign(source, properties) {
	    ...
	    for (var propertys in properties) {
	        if (properties.hasOwnProperty(propertys)) {
	            destination[propertys] = properties[propertys];
	        }
	    }
	    return destination;
	}
	    ...
	    return new Promise(function (resolve, reject) {
	        gateway.actions
	            ...
	            if (data["card-track"]) {
	                ...
	                var typee = typeByTrack(cardTrack);
	                resp.details = resp.details || {};
	                resp.details.cardType = typee ? typee.code : "unknown";
	            }
	            if (data["account-number"]) {
	                var accountNumber = data["account-number"].replace(/\D/g, "");
	                var last4a = accountNumber.substr(-4);
	                resp.details = resp.details || {};
	                resp.details.accountNumber =
	                    "*".repeat(accountNumber.length - 4) + last4a;
	                resp.details.accountLast4 = last4a;
	            }
	            resolve(resp);
	        })["catch"](reject);
	    });
	...
	var Card = /** @class */ (function () {
	    ...
	    Card.validateNumber = function (e) {
	        ...
	        if (new CardNumber$1().validate(value)) {
	            ...
	            var idh = target.getAttribute("data-id");
	            if (idh) {
	                postMessage.post({
	                    data: { valid: false },
	                    id: idh,
	    ...
	    Card.validateExpiration = function (e) {
	        ...
	        if (new Expiration$1().validate(value)) {
	            ...
	            classList.push("invalid");
	            var idg = target.getAttribute("data-id");
	            if (idg) {
	                postMessage.post({
	                    data: { valid: false },
	                    id: idg,
	...
	var IframeField = /** @class */ (function (_super) {
	    ...
	    IframeField.createField = function (id, name, type) {
	        ...
	        if (name === "card-track") {
	            ...
	            var messages = "Submit";
	            input.appendChild(document.createTextNode(messages));
	        }
	...
	var UIForm = /** @class */ (function () {
	    ...
	    UIForm.prototype.requestDataFromAll = function (target) {
	        ...
	        for (var _a = 0, fields_1 = fields; _a < fields_1.length; _a++) {
	            var typek = fields_1[_a];
	            if (typek === "submit") {
	                continue;
	            }
	            var field = this.frames[typek];
	            if (!field) {
	                continue;
	            }
	            postMessage.post({
	...
	function form(target, formOptions) {
	    ...
	    for (var i in fieldTypes) {
	        ...
	        elf.id = formOptions.prefix + type;
	        target.appendChild(elf);
	        fields[type] = {};
	...
	function trackReaderForm(target, formOptions) {
	    ...
	    for (var i in fieldTypes) {
	        ...
	        elc.id = formOptions.prefix + type;
	        target.appendChild(elc);
	        fields[type] = {};
	        fields[type].target = "#" + formOptions.prefix + type;
	        if (formOptions.placeholders && formOptions.placeholders[type]) {
	            fields[type].placeholder = formOptions.placeholders[type];
	...
	function form$1(target, formOptions) {
	    ...
	    for (var i in fieldTypes) {
	        ...
	        elb.id = formOptions.prefix + type;
	        target.appendChild(elb);
	        if (type === "account-type") {
	            ...
	            elb.appendChild(select);
	            continue;
	        }
	        if (type === "check-type") {
	            var selecta = document.createElement("select");
	            selecta.name = "check-type";
	            var defaultOptiona = document.createElement("option");
	            defaultOptiona.appendChild(document.createTextNode("-- Check Type --"));
	...
	function form$2(target, formOptions) {
	    ...
	    for (var i in fieldTypes) {
	        ...
	        ela.id = formOptions.prefix + type;
	        target.appendChild(ela);
	        if (type === "pin") {
	            var input = document.createElement("input");
	            input.name = "pin";
	            input.type = "tel";
	            ela.appendChild(input);
	            continue;
	        }
	        fields[type] = {};
	        fields[type].target = "#" + formOptions.prefix + type;
